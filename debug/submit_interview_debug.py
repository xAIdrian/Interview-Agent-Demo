...
# Constants
API_BASE_URL = "https://main-service-48k0.onrender.com/api"  # Update if your API is hosted elsewhere
...
def submit_interview_transcript(submission_id: str, transcript: str) -> bool:
    ...
    try:
        response = requests.post(
            ...
            json={"submission_id": str(submission_id), "transcript": transcript},
        )
        if response.status_code == 200:
...
def generate_transcript(questions: List[Dict[str, Any]]) -> str:
    ...
    print(
        "For each question, enter your response. Press Enter twice to move to the next question.\n"
    )
