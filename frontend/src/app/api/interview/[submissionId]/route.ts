...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const submissionResponse = await axios.get(`http://127.0.0.1:5000/api/submissions/${submissionId}`);
    const submission = submissionResponse.data;

    // Then fetch the campaign data using the campaign_id from the submission
    const campaignResponse = await axios.get(`http://127.0.0.1:5000/api/campaigns/${submission.campaign_id}`);
